...
Priority:
  NBA  → NBA.com direct → nba_api → DB historical → static fallback
  WNBA → DB current season → static fallback
...
# ─────────────────────────────────────────────
# ESPN — intentionally disabled for NBA
# ─────────────────────────────────────────────

def fetch_nba_espn() -> dict:
    # ESPN team endpoint doesn't expose opponent pts reliably
    # DB and NBA.com are more accurate sources
    return {}
...
def get_live_ratings(league: str, force_refresh: bool = False) -> dict:
    ...
    if league == "NBA":
        # Tier 1 — NBA.com direct
        ratings = fetch_nba_direct()

        # Tier 2 — nba_api package
        if not ratings:
            ratings = fetch_nba_via_nba_api()

        # Tier 3 — DB historical
        if not ratings:
            print("  [Ratings] Live fetch failed — trying DB...")
            try:
                from db_ratings import get_all_db_ratings
                ratings = get_all_db_ratings("nba")
                if ratings:
                    print(f"  [Ratings] DB returned {len(ratings)} NBA teams.")
            except Exception as e:
                print(f"  [Ratings] DB error: {e}")

        # Tier 4 — static fallback
        if not ratings:
            print("  [Ratings] All sources failed — using static fallback.")
            ratings = NBA_STATIC

    elif league == "WNBA":
        # Tier 1 — DB current season
        try:
            from db_ratings import get_all_db_ratings
            ratings = get_all_db_ratings("wnba")
            if ratings:
                print(f"  [Ratings] DB returned {len(ratings)} WNBA teams.")
        except Exception as e:
            print(f"  [Ratings] DB error: {e}")

        # Tier 2 — static fallback
        if not ratings:
            print("  [Ratings] DB failed — using static fallback.")
            ratings = WNBA_STATIC

        # Merge with static to fill any missing teams
        for team, rating in WNBA_STATIC.items():
