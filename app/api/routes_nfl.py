...
def _build_bets_for_game(home: str, away: str, pred, events_odds: list, min_edge: float) -> tuple:
    ...
    # overwrites pred's attributes in place — same reasoning as
    # routes_wnba.py's patch. Needed so log_prediction() can save both the
    # calibrated model_prob and the raw value; refitting the curve later
    # must train on raw values, never already-calibrated ones.
    ...
    pred.home_win_prob = apply_calibration(pred.home_win_prob, "moneyline", sport="nfl") * 100
    pred.away_win_prob = apply_calibration(pred.away_win_prob, "moneyline", sport="nfl") * 100
    pred.home_cover_prob = apply_calibration(pred.home_cover_prob, "spread", sport="nfl") * 100
    pred.away_cover_prob = apply_calibration(pred.away_cover_prob, "spread", sport="nfl") * 100
    pred.over_prob = apply_calibration(pred.over_prob, "total", sport="nfl") * 100
    pred.under_prob = apply_calibration(pred.under_prob, "total", sport="nfl") * 100
