...
def get_market_implied(events_odds: list, home: str, away: str) -> tuple:
    ...
    Uses median across bookmakers.
    Falls back to -110 (~52.4%).
    """
    ...
    return (
        home_probs[len(home_probs) // 2],
        away_probs[len(away_probs) // 2],
    )
...
def _get_spread_and_total(events_odds: list, home: str, away: str) -> tuple:
    ...
    for game in events_odds:
        ...
        return (
            ...
            total if total is not None else None,
        )
...
def wnba_edges(
    ...
    for event in events:
        ...
        bet_odds = (
            round(-(bet_prob / (100 - bet_prob)) * 100)
            if bet_prob >= 50
