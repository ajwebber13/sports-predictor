...
def get_team_stats(team_name: str) -> Optional[CFBTeamStats]:
    ...
    stats_data = _get(f"{ESPN_BASE}/teams/{team_id}/statistics")
    team_data  = _get(f"{ESPN_BASE}/teams/{team_id}")
    ...
    pts_off = all_stats.get("totalPointsPerGame",
              all_stats.get("points", 0) / max(wins + losses, 1))
    pts_def = all_stats.get("pointsAgainstPerGame",
              all_stats.get("pointsAgainst", 0) / max(wins + losses, 1))
    ...
    sacks_all  = round(all_stats.get("sacksAllowed", all_stats.get("sacks", 0)) / games, 2)
    sacks_for  = round(all_stats.get("sacks", 0) / games, 2)
    penalties  = round(all_stats.get("penalties", 0) / games, 1)

    # Fallback defaults if ESPN returns zeros
    if pts_off < 5:  pts_off = 28.0
    if pts_def < 5:  pts_def = 28.0
    if ypp_off < 2:  ypp_off = 5.5
    if ypp_def < 2:  ypp_def = 5.5
