...
def wnba_edges(simulations: int = Query(default=10000), min_edge: float = Query(default=3.0)):
    ...
    for event in events:
        ...
        if not home_stats or not away_stats:
            continue
        # Pull posted spread + totals from odds
        from services.odds_parser import parse_spread, parse_totals
        game_odds_obj = next(
            (g for g in events_odds
             if g.get("home_team") == home or g.get("away_team") == away),
            {}
        )
        spread_outcomes = parse_spread(game_odds_obj) or []
        totals_outcomes = parse_totals(game_odds_obj) or []

        posted_spread = None
        for o in spread_outcomes:
            if o.get("name") == home:
                posted_spread = o.get("point")
                break

        posted_total = None
        for o in totals_outcomes:
            if o.get("name") == "Over":
                posted_total = o.get("point")
                break

        # Re-run prediction with posted spread/total if available
        pred = engine.predict(
            home_stats=home_stats, away_stats=away_stats,
            spread_line=posted_spread or 0.0,
            over_under=posted_total or 164.0,
        ...
        # Predicted margin and spread edge
        pred_margin       = round(pred.projected_home - pred.projected_away, 1)
        spread_edge_home  = round(pred_margin - (posted_spread or 0), 1) if posted_spread else None
        spread_pick       = None
        spread_cover_prob = None
        if posted_spread is not None and abs(spread_edge_home) >= 3:
            if spread_edge_home > 0:
                spread_pick       = f"{home} -{abs(posted_spread)}"
                spread_cover_prob = pred.home_cover_prob
            else:
                spread_pick       = f"{away} +{abs(posted_spread)}"
                spread_cover_prob = pred.away_cover_prob
